...
public class UserService implements DatabaseConstants{
    ...
    public boolean getByLoginByContact(String contact, String pwd) throws SQLException {
        Criterias criterias = new Criterias();
        criterias.addCriteria(new Criteria("contact", "=", contact));
        criterias.addCriteria(new Criteria("password", "=", pwd));
        User user = userCRUD.read(criterias).get( 0 );
